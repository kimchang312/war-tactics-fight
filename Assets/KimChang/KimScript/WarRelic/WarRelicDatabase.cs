...
public static class WarRelicDatabase
{
    ...
    static WarRelicDatabase()
    {
        ...
        relics.Add(new WarRelic(16, "제국 수호자의 훈장", 10, "수호 스킬을 가진 아군 유닛의 장갑이 n 증가하고, 공격 받을때 데미지를 둘로 나눠서 받는다.", RelicType.ActiveState, () => MedalOfImperialGuard()));
        ...
        relics.Add(new WarRelic(30, "부러진 직검", 0, "아군이 공격시 낮은 확률로 데미지 일정 수치 감소.", RelicType.BattleActive, () => BrokenStraightSword()));
        ...
        relics.Add(new WarRelic(33, "갈라진 방패", 0, "아군이 피격시 낮은 확률로 장갑을 무시하고 데미지를 받음.", RelicType.BattleActive, () => SplitShield()));
        relics.Add(new WarRelic(34, "황폐한 깃발", 0, "사기가 감소할 때 일정 수치 더 감소함.", RelicType.SpecialEffect, () => WastedFlag()));
        relics.Add(new WarRelic(35, "", 10, "아군이 사망할수록 다른 아군 공격력 일정수치 증가, 마지막에 남은 아군의 공격력, 장갑, 기동성 일정수치 증가.", RelicType.BattleActive, () => Relic35()));
        relics.Add(new WarRelic(36, "", 10, "엘리트 스테이지 승리 시 전쟁 유산 보상이 한번 더 등장함.", RelicType.SpecialEffect, () => Relic36()));
        ...
        relics.Add(new WarRelic(39, "", 1, "모든 아군 유닛의 공격력 일정배수(5~10%) 증가.", RelicType.StateBoost, () => Relic39()));
        ...
        relics.Add(new WarRelic(42, "", 1, "일반, 엘리트 전투 시작 시 모든 적의 체력이 일정 배수(약 10%) 깎이고 시작.", RelicType.StateBoost, () => Relic42()));
        ...
        relics.Add(new WarRelic(44, "적장 정찰 보고서", 1, "엘리트 및 보스 스테이지에서 주는 피해 일정배수 증가.", RelicType.StateBoost, () => EnemyGeneralScoutReport()));
        relics.Add(new WarRelic(45, "실수한 발주 영수증", 1, "상점 리롤 시 이미 구매한 유닛, 전쟁 유산 칸이 다시 입고됨.", RelicType.SpecialEffect, () => MistakenOrderReceipt()));
        relics.Add(new WarRelic(46, "", 10, "유닛을 병종 한 종류로만 배치 시 아군 공격력 일정수치 증가.", RelicType.StateBoost, () => Relic46()));
        ...
        relics.Add(new WarRelic(53, "", 1, "전쟁 유산 보상의 선택지 한개 증가.", RelicType.SpecialEffect, () => Relic53()));
        relics.Add(new WarRelic(54, "", 10, "주는 모든 피해가 2배로 적용됨. 단, 이 전쟁 유산을 획득할 때 높은 확률로 랜덤한 다른 전설 전쟁 유산으로 대체됨.", RelicType.StateBoost, () => Relic54()));
        relics.Add(new WarRelic(55, "", 1, "아군 유닛 사망 시 전열 적 유닛에게 피해.", RelicType.BattleActive, () => Relic55()));
        relics.Add(new WarRelic(56, "", 10, "아군 유닛들의 체력, 공격력이 무작위로 분배됨.", RelicType.StateBoost, () => Relic56()));
        relics.Add(new WarRelic(57, "", 10, "전투에서 적 처치로도 사기를 일정 수치 회복하게 된다.", RelicType.BattleActive, () => Relic57()));
        relics.Add(new WarRelic(58, "", 10, "영웅 유닛 최대 보유수 1 증가.", RelicType.SpecialEffect, () => Relic58()));
        relics.Add(new WarRelic(59, "", 0, "병영 리롤 기회 -1.", RelicType.SpecialEffect, () => Relic59()));
        relics.Add(new WarRelic(60, "", 0, "군사 아카데미 리롤 기회 -1.", RelicType.SpecialEffect, () => Relic60()));
        relics.Add(new WarRelic(61, "", 10, "영웅 유닛을 보유하지 않을 시.", RelicType.SpecialEffect, () => Relic61()));
    ...
    {
        RogueLikeData.Instance.AddMyMultipleDamage(9f);
        RogueLikeData.Instance.AddEnemyMultipleDamage(9f);
    }
    ...
    private static void VeryLightMilitaryPants()
    {
        ...
        foreach (var unit in units)
        {
            if(unit.branchIdx !=3||unit.branchIdx != 5)
            {
                unit.mobility += 9;
            }
        }
    ...
    //정예 기병대 안장 아군 경기병 유닛의 공격력 n 증가, 아군의 모든 유닛이 경기병이라면 아군 유닛의 기동력이 n 증가하고
    ...
    private static void CreepyOrb()
    {
        ...
        if (curseCount > 0)
        {
            ...
            foreach (var unit in units)
            {
                unit.maxHealth +=  (curseCount * 9);
                unit.health = unit.maxHealth;
                unit.attackDamage += (curseCount * 9);
            }
            RogueLikeData.Instance.AllMyUnits(units); // 변경된 데이터 저장
        }
        return;
    ...
    private static void WornOutBoots()
    {
        ...
        foreach (var unit in units)
        {
            unit.mobility = UnityEngine.Mathf.Max(1, unit.mobility - 1);
        }
    ...
    private static void Relic39()
    {
        ...
        foreach (var unit in units)
        {
            unit.attackDamage = unit.attackDamage *1.09f;
        }
    ...
    private static void Relic42()
    {
        ...
        if(stage.type == RogueLikeData.StageType.Battle || stage.type == RogueLikeData.StageType.Elite)
        {
            ...
            foreach (var unit in units)
            {
                unit.maxHealth = unit.maxHealth * 0.91f;
                unit.health = unit.maxHealth;
            }
    ...
    private static void EnemyGeneralScoutReport()
    {
        ...
        {
            RogueLikeData.Instance.AddMyMultipleDamage(9);
        }
    }
