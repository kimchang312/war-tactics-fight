...
public static class WarRelicDatabase
{
    ...
    {
        relics.Add(new WarRelic(1, "할인패", 1, "용병단과 상단의 금화 가격이 20% 감소한다.", RelicType.SpecialEffect, () => DiscountCard()));
        relics.Add(new WarRelic(2, "연구 예산 지원금", 1, "군사 아카데미의 강화 비용이 20% 감소한다.", RelicType.SpecialEffect, () => SolidAnvil()));
        relics.Add(new WarRelic(3, "인내력의 깃발", 1, "기력이 감소할 때마다 25% 확률로 감소하지 않는다.", RelicType.SpecialEffect, () => FlagOfEndurance()));
        relics.Add(new WarRelic(4, "추가 보급 명령서", 1, "병영에서 유닛 훈련에 요구하는 턴이 1 감소한다. (1 미만으로 내려가지 않음)", RelicType.SpecialEffect, () => ExtraSupplyOrder()));
        relics.Add(new WarRelic(5, "도금 망원경", 1, "전투 시작 전, 아군 궁병 중 가장 앞에 있는 궁병에 한해서 사거리가 1 증가한다.", RelicType.StateBoost, () => GoldPlatedTelescope()));
        relics.Add(new WarRelic(6, "행운의 금화 주머니", 1, "얻는 금화량이 15% 증가한다.", RelicType.SpecialEffect, () => LuckyCoinPouch()));
        relics.Add(new WarRelic(7, "전설의 도굴꾼의 삽", 1, "다음에 전쟁 유산 보상을 얻을때 전설 전쟁 유산이 확정으로 등장한다. 보스에선 적용되지 않는다. 전설 전쟁 유산을 얻은 이후 이 유산은 소멸한다.", RelicType.SpecialEffect, () => LegendaryDiggerShovel()));
        relics.Add(new WarRelic(8, "기이한 돋보기", 20, "보물을 열거나, 보스에게 승리할 때마다 리롤을 1회 얻는다.", RelicType.SpecialEffect, () => ArchaeologyKit()));
        relics.Add(new WarRelic(9, "순금 검", 1, "소유 중인 금화 100당 아군 유닛의 공격력이 1% 증가한다.", RelicType.StateBoost, () => GoldenSword()));
        relics.Add(new WarRelic(10, "예리한 양날도끼", 10, "적과 아군이 받는 피해가 각 30% / 10% 증가한다.", RelicType.StateBoost, () => SharpDoubleAxe()));
       
        relics.Add(new WarRelic(11, "광기의 깃발", 10, "적이 받는 피해가 20% 증가하고, 아군 유닛의 장갑이 1 감소한다. (0 미만으로 내려가지 않음)", RelicType.StateBoost, () => FlagOfMadness()));
        relics.Add(new WarRelic(12, "혼돈의 깃발", 10, "아군의 배치가 무작위 순서로 배치된다. 아군 유닛의 체력과 공격력이 30% 증가하고, 아군 궁병의 사거리가 1 증가한다.", RelicType.StateBoost, () => FlagOfChaos()));
        ...
        relics.Add(new WarRelic(15, "가시 갑옷", 10, "중갑을 특성을 가진 아군 유닛에게 가시 특성을 부여한다. 원래 가시를 가진 유닛은 장갑이 2 증가한다.", RelicType.BattleActive, () => ReactiveThornArmor()));
        ...
        relics.Add(new WarRelic(17, "매우 가벼운 군복 바지", 10, "중보병과 기병을 제외한 아군 유닛의 기동력이 8 증가한다.", RelicType.StateBoost, () => VeryLightMilitaryPants()));
        ...
        relics.Add(new WarRelic(23, "해주 부적", 20, "저주등급 유산의 해로운 효과를 받지 않게 된다.", RelicType.AllEffect, () => HaejuAmulet()));
        ...
        relics.Add(new WarRelic(28, "하트 보석 목걸이", 20, "아군 전열 유닛의 체력이 0이 되는 공격을 받을 때 체력의 최대치까지 회복한다. 전투에 한번만 발동한다.", RelicType.BattleActive, () => HeartGemNecklace()));
        relics.Add(new WarRelic(29, "용기의 깃발", 20, "사기의 효과 적용 선이 플레이어에게 유리하게 바뀜. (일반 상태를 제외한 모든 효과의 적용 선이 일정 수치 감소.)", RelicType.SpecialEffect, () => HeartGemNecklace()));
        ...
        relics.Add(new WarRelic(35, "생존자의 넝마떼기", 20, "아군이 사망할수록 다른 아군 공격력 일정수치 증가, 마지막에 남은 아군의 공격력, 장갑, 기동성 일정수치 증가.", RelicType.BattleActive, () => Relic35()));
        relics.Add(new WarRelic(36, "정복자의 인장", 10, "엘리트 스테이지 승리 시 전쟁 유산 보상이 한번 더 등장함.", RelicType.SpecialEffect, () => Relic36()));
        relics.Add(new WarRelic(37, "맹인전사의 안대", 10, "적의 배치 정보가 모두 숨겨져 알 수 없게 되지만, 아군 유닛의 체력과 공격력이 20%, 아군 궁병의 사거리가 1 증가하고, 부대 상한이 3 증가한다.", RelicType.ActiveState, () => BlindWarriorEyepatch()));
        ...
        relics.Add(new WarRelic(41, "누군가의 무료 배식권", 1, "상단에 진입할 때 모든 아군 유닛의 기력이 1 회복한다.", RelicType.SpecialEffect, () => FreeMealTicket()));
        ...
        relics.Add(new WarRelic(45, "추가 징병 계획서", 1, "용병단의 판매 유닛 슬롯 수가 4 증가한다.", RelicType.SpecialEffect, () => MistakenOrderReceipt()));
        ...
        relics.Add(new WarRelic(50, "재상의 보증서", 1, "금화를 소모할 때, 500금화까지 빌려서 사용할 수 있게된다.", RelicType.SpecialEffect, () => CreditAuthorization()));
        ...
        relics.Add(new WarRelic(52, "두꺼운 전술 교범", 1, "군사 아카데미에서 강화할 수 있는 선택지가 하나 더 추가된다.", RelicType.SpecialEffect, () => Relic52()));
        ...
        relics.Add(new WarRelic(56, "혼돈의 주사위", 20, "아군 유닛들의 체력, 공격력이 매 전투마다 최소 60%에서 최대 200%까지 무작위로 분배된다.", RelicType.StateBoost, () => Relic56()));
        relics.Add(new WarRelic(57, "광전사의 머리칼", 10, "전투에서 적 처치로도 사기를 일정 수치 회복하게 된다.", RelicType.BattleActive, () => Relic57()));
        relics.Add(new WarRelic(58, "용사의 훈장", 20, "영웅 유닛 최대 보유수 1 증가.", RelicType.SpecialEffect, () => Relic58()));
        relics.Add(new WarRelic(59, "횡령 증거품", 0, "용병단, 상단, 군사 아카데미의 금화 가격이 20% 증가한다.", RelicType.SpecialEffect, () => Relic59()));
        relics.Add(new WarRelic(60, "승전보", 0, "군사 아카데미 리롤 기회 -1.", RelicType.SpecialEffect, () => Relic60()));
        relics.Add(new WarRelic(61, "무명의 군단 배지", 10, "영웅 유닛을 보유하지 않을 시 아군 유닛의 체력과 공격력이 20% 증가하고, 장갑이 2 증가한다.", RelicType.SpecialEffect, () => NamelessLegionBadge()));
        relics.Add(new WarRelic(62, "뜨거운 심장 모형", 1, "아군 유닛의 체력이 10% 증가한다.", RelicType.StateBoost,()=>HotHeartModel()));
        relics.Add(new WarRelic(63, "약자낙인 인두", 0, "아군 유닛의 체력이 15% 감소한다.", RelicType.StateBoost,()=>UnderDogStigma()));
        relics.Add(new WarRelic(64, "매우 진한 스프", 20, "아군 유닛의 체력과 공격력이 10%, 장갑과 기동력이 1 증가한다.", RelicType.StateBoost, () =>VeryThickSoup()));
        relics.Add(new WarRelic(65, "전쟁 군주의 투구", 20, "아군 유닛의 체력이 25% 증가한다.", RelicType.StateBoost, () =>WarlordHelm()));
        relics.Add(new WarRelic(66, "전쟁 군주의 검", 20, "아군 유닛의 공격력이 25% 증가한다.", RelicType.StateBoost, () =>WarlordSword()));
        relics.Add(new WarRelic(67, "확장 진형도", 1, "부대 상한이 1 증가한다.", RelicType.SpecialEffect, () =>ExpandedFormationDiagram()));
        relics.Add(new WarRelic(68, "전쟁 군주의 휘장", 20, "부대 상한이 3 증가한다.", RelicType.SpecialEffect, () =>WarlordInsignia()));
        relics.Add(new WarRelic(69, "전리품 주머니", 1, "약탈로 획득하는 금화가 20 증가하고, 전투 시작 시 약탈이 없는 무작위 유닛 둘에게 약탈을 부여한다.", RelicType.BattleActive, () =>LootBag()));
        relics.Add(new WarRelic(70, "전위대의 갑옷", 1, "아군의 첫 전열 유닛에 한해서 장갑이 3 증가한다.", RelicType.StateBoost, () =>VanguardArmor()));
        relics.Add(new WarRelic(71, "선봉대 군화", 1, "아군의 첫 전열 유닛에 한해서 기동력이 3 증가한다.", RelicType.StateBoost, () =>VanguardBoots()));
        relics.Add(new WarRelic(72, "녹슨 쇠말뚝", 1, "아군의 모든 공격마다 장갑을 무시하는 고정 피해 5가 추가된다.", RelicType.BattleActive, () =>RustyIronStake()));
        relics.Add(new WarRelic(73, "수상한 부등변다면체", 10, "전투에 아군을 도와주는 영웅 유닛을 무작위로 하나 소환한다. 단, 10% 확률로 적군에 소환된다.", RelicType.StateBoost, () => SuspiciousScalenePolyhedron()));
        relics.Add(new WarRelic(74, "뭐든지 들어있는 상자", 1, "모든 유산 중에 하나를 무작위로 얻는다.", RelicType.SpecialEffect, () =>AnythingBox()));
        relics.Add(new WarRelic(75, "신성한 문서", 1, "'순교' 기술이 다음 순서 유닛의 체력을 이 유닛의 공격력만큼 증가시킨다.", RelicType.BattleActive, () =>SacredDocument()));
        relics.Add(new WarRelic(76, "푯대", 1, "기동력으로 증가하는 회피율이 2배가 된다.", RelicType.BattleActive, () =>Signpost()));
        relics.Add(new WarRelic(77, "경량 갑옷", 1, "경갑 특성을 지닌 유닛의 회피율이 5% 증가한다.", RelicType.BattleActive, () =>LightWeightArmor()));
        relics.Add(new WarRelic(78, "뿔피리", 1, "돌격 피해 배수가 30% 증가한다.", RelicType.BattleActive, () =>Horn()));
        relics.Add(new WarRelic(79, "사리 유산", 10, "중첩된 횟수당 모든 유닛의 체력과 공격력이 1% 증가한다. 중첩은 고승이 적을 처치할 때마다 1, 전투에서 승리할 때마다 3 증가한다.", RelicType.StateBoost, () =>SariHeritage()));
        relics.Add(new WarRelic(80, "기이한 조각", 1, "획득 시 원하는 유닛에게 무한 특성을 부여한다.", RelicType.SpecialEffect, () =>StrangePiece()));
        relics.Add(new WarRelic(81, "합금 박차", 1, "내 첫 번째 유닛이 기병이라면 해당 유닛의 체력이 15% 증가하고, 강한 돌격 특성을 얻는다.", RelicType.StateBoost, () =>AlloySpur()));
        relics.Add(new WarRelic(82, "벼려진 마창", 1, "유닛의 공격력이 기동력에 비례해 증가한다.", RelicType.StateBoost, () =>ForgedStable()));
        relics.Add(new WarRelic(83, "반응 갑옷", 1, "유닛의 공격력이 장갑에 비례해 증가한다.", RelicType.StateBoost, () =>ReactiveArmor()));
        relics.Add(new WarRelic(84, "창술 교범", 1, "수비 태세의 패널티가 사라지며, 수비 태세 기술을 가지고 있는 유닛의 체력이 20 증가한다.", RelicType.StateBoost, () =>SpearManual()));
        relics.Add(new WarRelic(85, "치유석", 1, "치유량이 30% 증가한다.", RelicType.BattleActive, () =>HealingStone()));
        relics.Add(new WarRelic(86, "정예병 모집서", 1, "희귀도 1 유닛의 등장 확률이 15% 감소한다.", RelicType.SpecialEffect, () =>EliteRecruitment()));
    }
    ...
    private static void GoldenSword()
    {
        ...
        if (gold > 0)
        {
            ...
            foreach (var unit in units)
            {
                unit.attackDamage *= 100+(gold/100);
            }
        }
        RogueLikeData.Instance.AllMyUnits(units); // 변경된 데이터 저장
    }
    ...
    private static void Relic56()
    {
        ...
        foreach (var unit in units)
        {
            ...
            unit.maxHealth = MathF.Round(unit.maxHealth * healthMultiplier); // 원래 체력의 60~200% 적용
            unit.health = unit.maxHealth;
            unit.attackDamage = MathF.Round(unit.attackDamage * attackMultiplier); // 원래 공격력의 60~200% 적용
        }
    ...
    private static void ForgedStable()
    {
        ...
        foreach (var unit in units)
        {
            unit.attackDamage += MathF.Floor((unit.mobility * (25 / 9)) - (25 / 9)); 
        }
        RogueLikeData.Instance.AllMyUnits(units);
    }
    //반응 갑옷 83
    private static void ReactiveArmor()
    {
        ...
        foreach (var unit in units)
        {
            unit.attackDamage += MathF.Floor(unit.armor*1.7f);
        }
        RogueLikeData.Instance.AllMyUnits(units);
    }
    //창술 교범 84
